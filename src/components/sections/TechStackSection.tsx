...
const techSkillsData: TechSkill[] = [
  ...
  { id: 'ts5', name: 'Bash', category: 'Languages', icon: Settings, proficiency: 80 },
...
export default function TechStackSection() {
  ...
  return (
    ...
    >
      <div
        ref={circle1Ref}
        className="absolute -z-10 top-[10%] left-[-25%] w-[70rem] h-[60rem] md:w-[85rem] md:h-[75rem] bg-accent/20 dark:bg-accent/15 rounded-[60%/40%] filter blur-[230px] md:blur-[300px] opacity-50 dark:opacity-40"
      ></div>
      <div
        ref={circle2Ref}
        className="absolute -z-10 bottom-[5%] right-[-30%] w-[65rem] h-[70rem] md:w-[80rem] md:h-[85rem] bg-primary/20 dark:bg-primary/15 rounded-[40%/55%] filter blur-[220px] md:blur-[290px] opacity-45 dark:opacity-35"
      ></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {categories.map((category, catIndex) => (
            <AnimatedSection key={category} animationType="fadeInUp" delay={`delay-${200 + catIndex * 100}` as `delay-${number}`}>
              <Card className="h-full shadow-xl hover:shadow-2xl transition-shadow duration-300 bg-card/80 backdrop-blur-md border-primary/20">
                <CardHeader>
